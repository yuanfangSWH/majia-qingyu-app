	...
	methods: {
		...
		Verification: function() {
			if (this.selected) {
				...
				var parameter = '?type=sms';
				this.$api.apiPost('/reg/authphone', parameter, data, 'post', (status, code, res) => {
					if (status) {
						//登录成功后的token保存下来
		...
		loginData: function(code) {
			var parameter = '?code=' + code + '&phone=' + this.mobile;
